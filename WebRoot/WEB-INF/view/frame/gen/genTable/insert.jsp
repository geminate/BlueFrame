...
<!-- 标题 -->
<sitemesh:title>业务表配置 - 填写业务表信息</sitemesh:title>
...
<sitemesh:container>
	...
	<h3 class="page-title">
		业务表配置<small>&emsp;添加业务表&emsp;</small>
	</h3>
	<div class="row">
		<div class="col-md-12">
			<div class="portlet box green">
				...
				<div class="portlet-body form">
					<form action="${ctx}/frame/gen/genTable/insert" method="post" class="horizontal-form" data-validate>
						<div class="form-body">
								...
								<div class="col-md-4">
									<div class="form-group">
										...
										<div class="input-icon right">
											<i class="fa"></i>
											<input readonly="readonly" type="text" class="form-control" name="name" value="${genTable.name}">
										</div>
									</div>
								</div>
								<div class="col-md-4">
							...
							<h3 class="form-section">自定义列信息</h3>
							<div class="row">
								<div class="col-md-12">
												...
												<c:forEach items="${genTable.tableColumns}" var="item" varStatus="index">
													<c:if test="${item.isSystem eq '1'}">
														<tr>
															...
															<td style="word-break: break-word;">${item.name}</td>
															<td>
																<input readonly="readonly" name="tableColumns[${index.index}].comments" class="form-control input-sm" type="text" value="${item.comments}">
															</td>
															<td>
																<input type="hidden" name="tableColumns[${index.index}].jdbcType" value="${item.jdbcType}" />${item.jdbcType}</td>
															<td>
																<input type="hidden" name="tableColumns[${index.index}].javaType" value="${item.javaType}" />
															...
															<td>																
																<input readonly="readonly" class="form-control input-sm" type="text" value="${item.javaField}">
															</td>
															<td>
																<c:if test="${item.isPk eq '1'}">
																	<input type="hidden" name="tableColumns[${index.index}].isPk" value="1" />
																</c:if>
																<label class="mt-checkbox mt-checkbox-outline table-checkbox">
																	<input disabled="disabled" type="checkbox" value="1" ${item.isPk eq '1' ? 'checked' : ''} />
																	<span></span>
																</label>
															</td>
															<td>
																<c:if test="${item.isNull eq '1'}">
																	<input type="hidden" name="tableColumns[${index.index}].isNull" value="1" />
																</c:if>
																<label class="mt-checkbox mt-checkbox-outline table-checkbox">
																	<input disabled="disabled" type="checkbox" name="tableColumns[${index.index}].isNull" value="1" ${item.isNull eq '1' ? 'checked' : ''} />
																	<span></span>
																</label>
															</td>
															<td>

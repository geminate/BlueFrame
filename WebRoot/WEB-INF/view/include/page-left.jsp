...
<div class="page-sidebar-wrapper">
	<div class="page-sidebar navbar-collapse collapse">
		<ul class="page-sidebar-menu page-header-fixed" data-keep-expanded="false" data-auto-scroll="true" data-slide-speed="200" style="">
			...
			<c:forEach items="${fns:getCurrentPermissionList()}" var="level1">
				<c:if test="${(level1.parentId eq '1') and (level1.type eq 0)}">
					<li class="nav-item ${(i1 eq 0)?'start':''}">
						<a data-flag="${level1.permissionStr}"
							href="<c:if test="${empty level1.href}">javascript:;</c:if><c:if test="${not empty level1.href}">${ctx}${level1.href}</c:if>"
							...
							<ul class="sub-menu">
								<c:forEach items="${fns:getCurrentPermissionList()}" var="level2">
									<c:if test="${(level1.id eq level2.parentId) and (level1.type eq 0)}">
										<li class="nav-item">
											<a data-flag="${level1.permissionStr}"
												href="<c:if test="${empty level2.href}">javascript:;</c:if><c:if test="${not empty level2.href}">${ctx}${level2.href}</c:if>"
			...
			<li class="heading">
				<h3 class="uppercase">用户菜单</h3>
			</li>
			<li class="nav-item">
				<a href="javascript:;" class="nav-link nav-toggle"> <i class="icon-users"></i> <span class="title">个人中心</span> <span class="arrow open"></span>
				</a>
				<ul class="sub-menu">
					<li class="nav-item">
						<a href="${ctx}/frame/sys/userCenter" class="nav-link "> <i class="icon-home"></i> <span class="title">个人中心</span> <span class="selected"></span>
						</a>
					</li>
					<li class="nav-item  ">
						<a href="${ctx}/frame/sys/account" class="nav-link "> <i class="icon-settings"></i> <span class="title">账号设定</span>
						</a>
					</li>
				</ul>
			</li>
